...
//		Instances data = loader.getDataSet();
		...
		for (int i = 0; i < labelFiles.length; i++) {
			...
			// Load data
			Instances train_data = loadData(data, labelFiles[i]);
	...
	public static Instances loadData(Instances instances, String pathLabels)
			throws Exception {
		...
		removeString.setInputFormat(instances);
		Instances filteredData = Filter.useFilter(instances, removeString);
		// data.deleteStringAttributes();

		// System.out.println(labeles.toSummaryString());

		// Append label as class value
		Instances labeledData = Instances.mergeInstances(filteredData, instances);
